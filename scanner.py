"""
ALPHAEDGE PYTHON SCANNER v6.1 — PINE SCRIPT PARITY EDITION (AUDITED)
═══════════════════════════════════════════════════════════════
Mirrors AlphaEdge v6.3.2 Pine Script logic EXACTLY:
• Wilder's RMA for RSI/ATR/ADX
• Range Filter identical to Pine's rngfilt_va (VOLUME-direction based)
• Supertrend with HL2 + ratcheting bands (pandas 2.x safe)
...
v6.1 FIXES:
• Fixed range_filter to use volume direction (Pine's teth parameter)
• Fixed supertrend pandas 2.x compatibility (.iloc assignment)
• Fixed trend_up_value pandas 2.x compatibility
• Fixed smooth_range to match wbburgin library
• Added chop filter to signal gate (was declared but unused)
• Fixed AI text persistence
• Fixed resample deprecation warnings
• Fixed HTF None handling in counter-trend block
...
# ═══════════════════════════════════════════════
# CORE INDICATORS — PINE EXACT (Wilder's RMA)
# ═══════════════════════════════════════════════
...
def pine_supertrend(df, period=10, mult=3.0):
    """Pine Supertrend with ratcheting bands — pandas 2.x safe (numpy arrays internally)."""
...
# ═══════════════════════════════════════════════
# RANGE FILTER (Pine's rngfilt_va — VOLUME-DIRECTION BASED)
# ═══════════════════════════════════════════════
...
def range_filter(src, teth, rng):
    """
    Pine's rngfilt_va exact:
      if teth > teth[1]:
          rf = (src - srng) < rf[1] ? rf[1] : (src - srng)
      else:
          rf = (src + srng) > rf[1] ? rf[1] : (src + srng)

    teth = volume (what drives direction in AE).
    """
    n = len(src)
...
def trend_up_value(filt):
    """Pine's utils.trendUp — cumulative direction count (numpy-based, pandas 2.x safe)."""
    n = len(filt)
...
def get_htf_bias(symbol):
    """Returns True/False/None (None = insufficient data)."""
    try:
...
def analyze_symbol(symbol, tf_config, htf_bull, mtf_sum, last_signal_info=None):
    ...
    try:
        ...
        # AE Range Filter (volume-driven per Pine)
        ...
        bull += 1 if bool(sqz_bull_break.iloc[-1]) else 0
        ...
        # Counter-trend block (handle None HTF safely)
        ...
        # Chop filter (was missing from previous version)
        chop_ok = True
        if USE_CHOP_FILTER and last_signal_info:
            try:
                ...
                if prev_price and prev_atr:
                    min_move = prev_atr * CHOP_ATR_MULT
                    if abs(bar_price - prev_price) < min_move:
                        chop_ok = False
            except:
                pass
        ...
        if not raw_buy and not raw_sell:
            ...
            if not chop_ok: return None, "chop filter (too close to last signal)"
            return None, None
        ...
        if live_price and not sanity_check_price(symbol, live_price, daily_close):
            return None, f"bad data (live=${live_price:.2f}, daily=${daily_close:.2f})"
        ...
        if signal == 'BUY':
            strong_trend = (ema_bull and bar_price > ema50_v and 22 < adx_val < 55 and plus_di > minus_di)
        else:
            strong_trend = (not ema_bull and bar_price < ema50_v and 22 < adx_val < 55 and minus_di > plus_di)
...
def create_trade(sig):
    return {
        ...
        'ai_text_at_entry': sig.get('ai_text'),   # Preserve AI for history
...
def format_open_positions_summary(trades):
    ...
    for k, trade in active:
        ...
        if not live:
            enriched.append({
                'trade': trade, 'current': None, 'r_mult': 0,
    ...
    def _row(e, emphasis=False):
        t = e['trade']
        em = t.get('emoji', '📈')
        dir_em = "🟢" if t['signal'] == 'BUY' else "🔴"
        r_str = f"*{fmt_r(e['r_mult'])}*" if emphasis else fmt_r(e['r_mult'])
        line = f"  {em} {dir_em} *{t['symbol']}* `{t.get('tf_label', t['tf'])}` {r_str}"
        line += f" • {time_ago(t['opened_at'])}"
        if emphasis and e.get('status'):
            line += f" • {e['status']}"
        return line + "\n"
...
# ═══════════════════════════════════════════════
# SIGNAL TRACKING (for chop filter)
# ═══════════════════════════════════════════════

def get_last_signal_info(cache, symbol, tf):
    """Returns last signal info for chop filter, or None."""
...
def main():
    ...
    print(f"\n{'=' * 60}")
    print(f"AlphaEdge v6.1 PINE-PARITY @ {fmt_datetime()}")
    ...
    logging.info(f"Scan v6.1 | Session: {session} | Active: {len(active_list)}")
    ...
    for sym in active_list:
        ...
        for tf_cfg in TIMEFRAMES:
            ...
            # Load last signal for chop filter
